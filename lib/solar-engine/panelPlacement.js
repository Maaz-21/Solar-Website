...
export function generatePanelLayout({
  ...
  panelOrientation = "portrait", roofAreaM2 = 0, maxPanelCount = null,
}) {
  if (!usableGeometry) {
  ...
  const panels = [];
  const panelLimit = Number.isFinite(maxPanelCount) && maxPanelCount > 0
    ? Math.floor(maxPanelCount)
    : Infinity;
  ...
  while (y < maxLat) {
    ...
    while (x < maxLng) {
      try {
        ...
        if (panels.length < panelLimit && turf.booleanContains(usable, rect)) {
